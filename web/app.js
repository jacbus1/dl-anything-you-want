...
const t=isEnglish?{
  invalidConfig:'Invalid API configuration',invalidSource:'Use a Facebook, Instagram, Threads or TikTok HTTPS video URL.',
  files:n=>`${n} file${n===1?'':'s'}`,note:'Each link works once and expires in about one minute.',photo:'Photo',video:'Video',save:'Save',opened:'Opened',
  ...
  working:'Downloading…',workingNote:'Resolving video…',noApi:'This address has no download API. The GitHub Pages site is the frontend only.',
  ...
  invalidConfig:'API 設定無效',invalidSource:'只接受 Facebook／Instagram／Threads／TikTok 的 HTTPS 影片連結。',
  files:n=>`${n} 個檔案`,note:'每條連結只能使用一次，約一分鐘內有效。',photo:'相片',video:'影片',save:'儲存',opened:'已開啟',
  unsafe:'後端返回不安全的下載路徑。',openedNote:'瀏覽器已開啟下載；若連結過期，請重新解析。',
  clipboard:'瀏覽器未授予剪貼簿權限，請手動貼上。',consent:'請先確認你擁有下載權利。',api:'請先設定有效的 API 網址。',
  working:'下載中…',workingNote:'正在解析影片…',noApi:'此地址沒有下載 API；GitHub Pages 只提供介面。',failed:'無法建立下載連結。',
...
function safeSource(input) {
  ...
  const hosts=['facebook.com','www.facebook.com','m.facebook.com','fb.watch','instagram.com','www.instagram.com','m.instagram.com','threads.com','www.threads.com','threads.net','www.threads.net','tiktok.com','www.tiktok.com','m.tiktok.com','vm.tiktok.com','vt.tiktok.com'];
  if (u.protocol!=='https:' || u.username || u.password || u.port || !hosts.includes(u.hostname)) throw new Error(t.invalidSource);
  return u.href;
}
function showResults(data) {
  ...
  for (const item of data.items) {
    ...
    const detail=document.createElement('small'); detail.textContent=item.type==='photo'?t.photo:t.video;
...
(async()=>{
  ...
  try{
    ...
    $('connection').textContent=d.instagram==='not-configured'?t.engineOff:t.online; $('connection').classList.add('ok');
