...
export function normalizeSource(input) {
  if (typeof input !== 'string' || input.length > 2048 || /[\s\\\x00-\x1f]/.test(input.trim()))
    throw new AppError('INVALID_URL', 'Paste a complete, valid HTTPS post URL.');
  let u;
  try { u = new URL(input.trim()); } catch { throw new AppError('INVALID_URL', 'The URL is invalid.'); }
  if (u.protocol !== 'https:' || u.username || u.password || u.port)
    throw new AppError('INVALID_URL', 'Only HTTPS URLs without credentials or custom ports are accepted.');
  let m;
  if (['instagram.com', 'www.instagram.com', 'm.instagram.com'].includes(u.hostname)) {
    ...
    // Instagram profile grids may expose post links as /handle/reel/id.
    m = u.pathname.match(/^\/[A-Za-z0-9._]{1,64}\/(p|reel|reels|tv)\/([A-Za-z0-9_-]{3,64})\/?$/);
    if (m) return {platform: 'instagram', id: m[2], url: `https://www.instagram.com/${m[1] === 'reels' ? 'reel' : m[1]}/${m[2]}/`};
  ...
  throw new AppError('UNSUPPORTED_URL', 'Use a public Facebook, Instagram, Threads, TikTok or YouTube media URL.');
}
...
export function mediaItem(url, type = 'video', index = 1, cobaltBase = '') {
  if (!isMediaURL(url, cobaltBase)) throw new AppError('UNSAFE_MEDIA', 'The source returned a media URL outside the allowlist.', 502);
  if (!['video', 'photo', 'audio'].includes(type)) throw new AppError('UNSUPPORTED_MEDIA', 'This media format is not supported.', 422);
...
export function parseCobalt(data, cobaltBase, requestedFormat = 'mp4') {
  if (!data || typeof data !== 'object') throw new AppError('UPSTREAM_FORMAT', 'The download engine returned an invalid response.', 502);
  if (data.status === 'error') throw new AppError('SOURCE_UNAVAILABLE', 'The download engine could not fetch the public media. It may require login, be deleted or be rate limited.', 422);
  if (data.status === 'picker' && Array.isArray(data.picker) && data.picker.length && data.picker.length <= 20)
    return data.picker.map((x,i) => {
      if (!['photo','video'].includes(x.type)) throw new AppError('UNSUPPORTED_MEDIA','This media format is not supported.',422);
  ...
  if (['redirect', 'tunnel'].includes(data.status) && typeof data.url === 'string') {
    ...
    const audio = requestedFormat === 'mp3' || /\.mp3$/i.test(data.filename || '');
    return [mediaItem(data.url, photo ? 'photo' : audio ? 'audio' : 'video', 1, cobaltBase)];
  }
  throw new AppError('UPSTREAM_FORMAT', 'The engine returned an unsupported result.', 502);
}
...
export function parseThreadsHTML(html, id) {
  if (typeof html !== 'string' || Buffer.byteLength(html) > 4 * 1024 * 1024)
    throw new AppError('PAGE_TOO_LARGE', 'The source page is too large.', 502);
  ...
  function scan(root) {
    ...
    while (stack.length) {
      ...
      if (++visited > 100000) throw new AppError('PAGE_TOO_COMPLEX', 'The source data is too complex.', 502);
  ...
  for (const post of found) {
    if (post.is_private === true || post.user?.is_private === true)
      throw new AppError('PRIVATE_CONTENT', 'Private content is not supported.', 403);
  ...
  if (!unique.length) throw new AppError('THREADS_UNAVAILABLE', 'No video or image was found on the public page. It may require login, have changed, or contain no downloadable media.', 422);
...
export class TicketStore {
  ...
  issue(item) {
    this.purge();
    if (this.items.size >= this.max) throw new AppError('BUSY', 'The service is busy. Try again shortly.', 503);
  ...
  take(id) {
    ...
    if (!item) throw new AppError('LINK_EXPIRED', 'The link was used or expired. Resolve it again.', 410);
    return item;
  }
}
