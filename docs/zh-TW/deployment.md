...
Docker 並非必要。先安裝 FFmpeg，再依 [Cobalt 官方無 Docker 步驟](https://github.com/imputnet/cobalt/blob/main/docs/run-an-instance.md)以 Node.js、Git、pnpm 啟動 Cobalt，在 `.env` 設定 `COBALT_URL=http://localhost:9000/`，再執行 `npm start`。宿主機連接埠只綁定 loopback；公開前應加上 HTTPS gateway 及防濫用控制。
