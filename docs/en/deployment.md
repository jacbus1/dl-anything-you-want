...
Docker is optional. Install FFmpeg, follow the [official non-Docker Cobalt steps](https://github.com/imputnet/cobalt/blob/main/docs/run-an-instance.md) with Node.js, Git and pnpm, set `COBALT_URL=http://localhost:9000/` in `.env`, then run `npm start`. The host port is loopback-only. Use an HTTPS gateway and abuse controls before public exposure.
